...
import (
	...
	"github.com/koh-yoshimoto/proglog/internal/loadbalance"
...
func TestPikcerNoSubConnAvailable(t *testing.T) {
...
func TestPikckerProducesToLeader(t *testing.T) {
	picker, subConns := setupTest()
	info := balancer.PickInfo{
		FullMethodName: "/log.vX.Lof/Produce",
	}
	for i := 0; i < 5; i++ {
...
func TestPikckerConsumesToLeader(t *testing.T) {
	picker, subConns := setupTest()
	info := balancer.PickInfo{
		FullMethodName: "/log.vX.Lof/Consume",
	}
	for i := 0; i < 5; i++ {
